...
def _headers():
    return {"Authorization": f"Bearer {config.CHIRPSTACK_API_KEY}", "Content-Type": "application/json"}


def _get(path, params=None):
    r = requests.get(f"{config.CHIRPSTACK_REST_URL}{path}", headers=_headers(), params=params, timeout=15)
    r.raise_for_status()
    return r.json()


def _post(path, body):
    r = requests.post(f"{config.CHIRPSTACK_REST_URL}{path}", headers=_headers(), json=body, timeout=15)
    r.raise_for_status()
    return r.json() if r.text else {}


def _delete(path):
    r = requests.delete(f"{config.CHIRPSTACK_REST_URL}{path}", headers=_headers(), timeout=15)
    r.raise_for_status()
...
def bootstrap_chirpstack():
    ...
    if not config.CHIRPSTACK_REST_URL or not config.CHIRPSTACK_API_KEY:
    ...
    try:
        ...
        if not tenant_id:
            existing = _get("/api/tenants", params={"search": config.CHIRPSTACK_TENANT_NAME, "limit": 10})
            match = next((t for t in existing.get("result", []) if t["name"] == config.CHIRPSTACK_TENANT_NAME), None)
            if match:
                tenant_id = match["id"]
            else:
                created = _post("/api/tenants", {"tenant": {
                    "name": config.CHIRPSTACK_TENANT_NAME,
                    "canHaveGateways": True,
                    "maxDeviceCount": 0,
                    "maxGatewayCount": 0,
                }})
                tenant_id = created["id"]
                log.info("Created ChirpStack tenant %s", config.CHIRPSTACK_TENANT_NAME)
        ...
        if not profile_id:
            ...
            if match:
                ...
                created = _post("/api/device-profiles", {"deviceProfile": {
                    ...
                    "tenantId": tenant_id,
                    "region": config.CHIRPSTACK_REGION,
