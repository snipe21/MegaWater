...
def init_db():
    ...
    with get_conn() as conn:
        ...
        -- One row per registered device/authenticator. A user can have several
        -- (phone fingerprint, laptop Face ID, etc). credential_id/public_key
        -- are base64url text - never raw bytes - to keep this a plain TEXT column.
        ...
        -- Short-lived server-side challenges for the registration/login
        -- ceremonies. 'login' rows have no user_id yet - discoverable
        -- (resident-key) credentials mean the browser prompts biometrics
        -- first and tells us who logged in afterwards, so nothing about the
        -- device/account is known until the credential comes back.
