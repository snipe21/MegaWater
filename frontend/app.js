...
function renderShell(role, appName, navItems, activeIdx) {
  ...
  const shell = el(`
    <div class="layout">
      <div class="sidebar">
        ...
        ${
          webauthnSupported()
            ? `<div class="nav-item" data-nav="biometric"><span class="nav-dot"></span>🔐 Biometric Login</div>`
            : ""
        }
  ...
  const biometricNav = shell.querySelector('[data-nav="biometric"]');
  if (biometricNav) biometricNav.onclick = () => openBiometricPanel();
...
async function renderSuperAdmin() {
  const activeIdx = window.location.hash === "#units" ? 1 : window.location.hash === "#settings" ? 2 : 0;
  ...
  const groups = [
    ...
    },
    {
      ...
      fields: [
        ...
        ["digest_send_time", "Digest Send Time (HH:MM)", "text"],
      ],
    },
  ];
