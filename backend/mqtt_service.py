...
def _handle_uplink(payload: dict):
    ...
    if settings.get("enable_ha_mqtt_discovery"):
        try:
            import ha_discovery
...
def start_mqtt_service():
    global _client
    if not settings.get("chirpstack_mqtt_host"):
        log.warning("chirpstack_mqtt_host not configured — MQTT service not started")
        return None

    client = mqtt.Client(callback_api_version=mqtt.CallbackAPIVersion.VERSION2)
    if settings.get("chirpstack_mqtt_user"):
        client.username_pw_set(settings.get("chirpstack_mqtt_user"), settings.get("chirpstack_mqtt_pass"))
    ...
    def run():
        while True:
            try:
                client.connect(settings.get("chirpstack_mqtt_host"), settings.get("chirpstack_mqtt_port"), keepalive=60)
                client.loop_forever()
            except Exception as e:
