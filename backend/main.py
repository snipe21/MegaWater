...
def startup():
    ...
    digest_scheduler.start_digest_scheduler()
...
@app.post("/api/super/digest/send-test")
def api_send_test_digest(user=Depends(require_super_admin)):
    from datetime import datetime, timedelta
    yesterday = datetime.utcnow().date() - timedelta(days=1)
    ok = digest.compose_and_send_digest(for_date=yesterday)
    log_action(user, "digest_test_sent", "config", "digest", after={"ok": ok})
    if not ok:
        raise HTTPException(
            status_code=400,
            detail="Digest not sent - check Telegram Bot Token and Telegram Digest Chat ID are both set correctly.",
        )
    return {"ok": True}
