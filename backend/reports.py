...
def _org_name():
    return settings.get("building_name") or settings.get("product_name")
...
def _topups_between(conn, unit_id, start_iso, end_iso):
    rows = conn.execute(
        """SELECT * FROM topup_transactions WHERE unit_id=? AND created_at BETWEEN ? AND ?
           ORDER BY created_at""",
        (unit_id, start_iso, end_iso),
    ).fetchall()
...
def build_unit_usage_pdf(unit_id: str, start_iso: str, end_iso: str, period_label: str) -> bytes:
    with get_conn() as conn:
        ...
        topups = _topups_between(conn, unit_id, start_iso, end_iso)
    ...
    elements = [
        Paragraph(f"{_org_name()} - Water Usage Report",
    ...
    if topups:
        ...
        elements.append(Paragraph("Prepaid Top-Ups This Period", styles["Heading3"]))
        rows = [["Date", "Amount (R)", "Liters Loaded"]]
        total_r, total_l = 0.0, 0.0
        for t in topups:
            rows.append([t["created_at"][:16].replace("T", " "), f"{t['amount_rand']:,.2f}", f"{t['liters_purchased']:.1f}"])
            total_r += t["amount_rand"]
            total_l += t["liters_purchased"]
        rows.append(["TOTAL", f"{total_r:,.2f}", f"{total_l:.1f}"])
        tuptable = Table(rows, colWidths=[60 * mm, 40 * mm, 40 * mm])
        tuptable.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#2dd4a7")),
            ("TEXTCOLOR", (0, 0), (-1, 0), colors.HexColor("#051a13")),
            ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
            ("GRID", (0, 0), (-1, -1), 0.5, colors.grey),
...
def build_building_summary_pdf(start_iso: str, end_iso: str, period_label: str) -> bytes:
    ...
    styles = getSampleStyleSheet()
    elements = [
        Paragraph(f"{_org_name()} - Building Water Usage Summary",
                  styles["Title"]),
        Paragraph(f"Period: {period_label} ({start_iso[:10]} to {end_iso[:10]})", styles["Normal"]),
        Paragraph(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}", styles["Normal"]),
        Spacer(1, 8 * mm),
...
def build_statement_pdf(period_id: str) -> bytes:
    ...
    styles = getSampleStyleSheet()
    elements = [
        Paragraph(f"{_org_name()} - Water Statement",
                  ParagraphStyle("t", parent=styles["Title"], fontSize=18)),
        Spacer(1, 4 * mm),
        Paragraph(f"Unit: <b>{unit['unit_number']}</b>", styles["Normal"]),
