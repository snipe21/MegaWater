...
def _headers():
    return {"Authorization": f"Bearer {settings.get("chirpstack_api_key")}", "Content-Type": "application/json"}


def _get(path, params=None):
    r = requests.get(f"{settings.get("chirpstack_rest_url")}{path}", headers=_headers(), params=params, timeout=15)
    if not r.ok:
...
def _post(path, body):
    r = requests.post(f"{settings.get("chirpstack_rest_url")}{path}", headers=_headers(), json=body, timeout=15)
    if not r.ok:
...
def _delete(path):
    r = requests.delete(f"{settings.get("chirpstack_rest_url")}{path}", headers=_headers(), timeout=15)
    r.raise_for_status()
